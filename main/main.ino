...
#include <SPI.h>
#include <math.h>
#include "FspTimer.h" // <-- Added for non-blocking hardware audio timer
...
#define CLOSE_DISTANCE 200      // mm
#define DARK_THRESHOLD 500      // LDR value
#define BORED_TIME 10 * 1000    // 30 seconds
#define SAD_TIME 30*1000      // 2 minutes
#define IMU_SPIKE_THRESHOLD 20  // deg/s or similar
...
const unsigned char* volatile currentAudioData = nullptr;
volatile unsigned int currentAudioLength = 0;
volatile unsigned int currentAudioIndex = 0;
...
// ======================================================
// NON-BLOCKING AUDIO TIMER ISR
// ======================================================
// This runs 8000 times a second in the background automatically
void audio_timer_callback(timer_callback_args_t __attribute((unused)) *p_args) {
  if (isAudioPlaying) {
    int sample = pgm_read_byte(&currentAudioData[currentAudioIndex]);
    ...
    // Stop playing when we reach the end of the array
...
// Rewritten playSound function that triggers the hardware timer
void playSound(int soundIndex) {
  if (soundIndex < 0 || soundIndex >= TOTAL_SOUNDS) {
    Serial.println("Error: Sound index out of range!");
    return;
  ...
  // Momentarily pause background playback to avoid glitches while updating pointers
  isAudioPlaying = false; 

  // Load up the new sound data
  currentAudioData = (const unsigned char*) pgm_read_ptr(&sound_table[soundIndex]);
  currentAudioLength = pgm_read_word(&sound_lengths[soundIndex]);
  currentAudioIndex = 0;

  // Let the background interrupt take over!
  isAudioPlaying = true; 
}
...
void setup()
{
  ...
  if (tindex >= 0) {
    audioTimer.begin(TIMER_MODE_PERIODIC, timer_type, tindex, 8000.0, 0.0, audio_timer_callback);
    audioTimer.setup_overflow_irq();
    audioTimer.open();
    audioTimer.start();
  } else {
    Serial.println("Failed to allocate hardware timer for audio!");
  }
...
void loop()
{
  ...
  if (isOn){
    ...
    if (emotion != last_emotion){
      drawEmotion(emotion);
      playSound(emotion); // <-- Uncommented this so it automatically plays!
    }
    if (emotion == HAPPY)
    {
      bobMotor(motor2);
